...
while (defined($line = <>)) {
    ...
    my $term_date = $term[0];
    my $term_time_end = $term[2];

    # convert to seconds
    my @term_end_array = split /:/, $term_time_end;
    ...
    if ($term_date ne $locked_date || $locked_seconds >= $term_deadline_seconds) {
        print "$jmbag $last_name $first_name - PROBLEM: $term_date $term_time_end --> $locked_date $locked_time\n";
    }
}
